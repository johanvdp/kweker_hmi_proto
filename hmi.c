...
static uint16_t hmi_get_control_value_y(lv_obj_t *bar, int16_t value) {
...
bool hi, bool lo) {
	...
	uint16_t pv_y = hmi_get_control_value_y(bar, pv);
	...
	uint16_t sv_y = hmi_get_control_value_y(bar, sv);
...
static void hmi_create_control(hmi_control_t *target, lv_obj_t *parent,
	...
	uint16_t pv_y = hmi_get_control_value_y(bar, min);
	lv_obj_set_pos(label_pv, bar_x + bar_width + 2, pv_y);
	...
	uint16_t label_width = lv_obj_get_width(label_sv);
	uint16_t sv_y = hmi_get_control_value_y(bar, min);
	lv_obj_set_pos(label_sv, bar_x - label_width - 2, sv_y);
...
static lv_obj_t* hmi_create_tab_control(lv_obj_t *parent) {
	...
	hmi_create_control(&hmi_control_temperature, tab, 10, 10,
			"Temperature [°C]", 0, 50);
	hmi_update_control(&hmi_control_temperature, 26, 30, false, true);

	hmi_create_control(&hmi_control_humidity, tab, 10 + HMI_CONTROL_W, 10,
			"Humidity [%RH]", 0, 100);
	hmi_update_control(&hmi_control_humidity, 46, 40, true, false);

	hmi_create_control(&hmi_control_co2, tab, 10 + HMI_CONTROL_W * 2, 10,
			"CO2 conc. [ppm]", 0, 2000);
	hmi_update_control(&hmi_control_co2, 415, 800, false, true);
